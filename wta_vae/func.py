from network import BasicVae, LWTAVae
import numpy as np
import os
...
def train_autoencoder(session, training_data, validation_data,
    ...
    if layer_structure == 'none':
        autoencoder = BasicVae()
    else:
        autoencoder = LWTAVae(layer_structure=layer_structure)
    ...
    x_recon, mu, logvar = autoencoder.forward(x)
    loss = loss_function(x_recon, x, mu, logvar, batch_size)
...
def reconstruct(mnist_index, model_path, validation_data, output_path, layer_structure):
    ...
    with tf.Session() as session:
        ...
        if layer_structure == 'none':
            autoencoder = BasicVae(load=True, graph=graph)
        else:
            autoencoder = LWTAVae(layer_structure=layer_structure,
